...
def benchmark_samplers_Rosenbrock_general(dim=2, n_samples=10000, burn_in=1000, sigma=0.7, a = 1.0, b = 100.0, affine = False):
    ...
    thin = 10
    total_keep = (n_samples + burn_in) // thin
    
    # Define samplers to benchmark - parameters tuned for the ring distribution
    samplers = {
        "HMC n=20": lambda: hmc_sst(log_density, initial, total_keep, gradient, epsilon=0.02, L=20, n_chains=200, n_warmup = 1500, n_thin = thin),
        "HMC n=5":  lambda: hmc_sst(log_density, initial, total_keep, gradient, epsilon=0.08, L=5,  n_chains=200, n_warmup = 1500, n_thin = thin),
        "Hamiltonian Walk Move n=20": lambda: hamiltonian_walk_move_sst(
            gradient, potential, initial, total_keep, n_thin = thin,
            n_chains_per_group=100, epsilon=0.02, n_leapfrog=20, beta=1.0, n_warmup = 1500
        ),
        "Hamiltonian Walk Move n=5": lambda: hamiltonian_walk_move_sst(
            gradient, potential, initial, total_keep, n_thin = thin,
            n_chains_per_group=100, epsilon=0.08, n_leapfrog=5, beta=1.0, n_warmup = 1500
        ),
        "HMC n=20 tuned": lambda: hmc_chees(log_density_jax, initial, total_keep, n_thin = thin, epsilon = 0.02, L = 20, n_chains = 200, n_warmup=1500),
        "HMC n=5 tuned": lambda: hmc_chees(log_density_jax, initial, total_keep, epsilon = 0.08, n_thin = thin, L = 5, n_chains = 200, n_warmup=1500),
        "Hamiltonian Walk Move n=20 tuned": lambda: hamiltonian_walk_chees(log_density_jax, initial, total_keep, n_thin = thin, epsilon = 0.02, L=20, n_walkers = 200, n_warmup = 1500),
        "Hamiltonian Walk Move n=5 tuned": lambda: hamiltonian_walk_chees(log_density_jax, initial, total_keep, n_thin = thin, epsilon = 0.08, L=5, n_walkers = 200, n_warmup = 1500),
    }
    ...
    for name, sampler_func in samplers.items():
        ...
        try:
            ...
            burn_keep = burn_in // thin

            post_burn_in_samples = samples[:, burn_keep:, :]
            burn_in_samps = samples[:, :burn_keep, :]
            series = post_burn_in_samples
            
            dstep = np.linalg.norm(np.diff(series[..., :min(2, dim)], axis=1), axis=2)
            print("  median step size:", np.median(dstep), " max step:", np.max(dstep))
            # Flatten samples from all chains
            flat_samples = series.reshape(-1, dim)   # thinned only
            ...
            burn_in_samps = np.asarray(burn_in_samps, dtype=np.float32)
            flat_samples = series.reshape(-1, dim)
            ...
            acf = np.zeros(100)
            tau, ess = np.nan, np.nan
            series = np.zeros((1, 1, dim))
            burn_in_samps = np.zeros((1, 1, dim))
            eps_final = 0.0
            eps_hist = np.array([0.0])
            n_leapfrog = n_warmup = target_accept = gamma = t0 = kappa = None
...
def plot_Rosenbrock_results(results, log_density, dim=2, sigma=0.7, transform = {'affine':False}):
    ...
    fig.tight_layout()
    fig.savefig(f"RosenbrockTunedResults/{dim}d{rotstring}/Rosenbrock_overlay_first2.png", dpi=220, bbox_inches="tight")
    plt.close()
    ...
    plt.legend()
    plt.savefig(f'RosenbrockTunedResults/{d}d{rotstring}/StepSize.pdf')
    plt.close()
...
#make directories
outdir = f"RosenbrockTunedResults/{d}d{afstring}"
corner_dir = f"RosenbrockTunedResults/{d}d{afstring}/corner"
trends_dir = f"RosenbrockTunedResults/{d}d{afstring}/trends"
