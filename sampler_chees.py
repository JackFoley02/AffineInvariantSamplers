...
def chees_L(state, epsilon, jitter = 0.6, use_bar = False, max_L = 100, apply_jit=True):
...
def hmc_sample( #function to perform a single hmc step after warmup
    key,
    q0,
    log_prob,
    grad_U,
    ...
    def step(carry, _):
        ...
        q_new, accept, accept_prob, log_alpha, q_prop, vel = hmc_step(
             subkey,
             q,
             log_prob,
             grad_U,
        ...
        return (key, q_new), (q_new, accept)
    
    (key, q_final), (all_samps, all_accepts) = jax.lax.scan(
         step,
         (key, q0),
         xs = None,
         length = steps
